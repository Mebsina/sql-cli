...
class TestSqlConnection:
    ...
    @pytest.mark.parametrize(
        "test_id, description, sql_connected, expected_result",
        [
            (1, "Disconnect success", True, True),
            (2, "Disconnect when not connected", False, False),
        ],
    )
    @patch("opensearchsql_cli.sql.sql_connection.console")
    def test_disconnect(
        ...
        """
        Test the disconnect method of SqlConnection.
        """
        print(f"\n=== Test Case #{test_id}: {description} ===")

        # Create connection instance with mocked SQL library
        connection = SqlConnection()
        connection.sql_connected = sql_connected
        connection.opensearch_connected = sql_connected

        if sql_connected:
            connection.sql_lib = mock_java_gateway
        ...
        # Verify result
        assert result == expected_result
        assert connection.sql_connected == False
        assert connection.opensearch_connected == False

        # Verify Java gateway call if connected
        if sql_connected:
            mock_java_gateway.entry_point.disconnect.assert_called_once()
